...
def plot_three_component_seismogram(stream, axZ, axN, axE):
    ...
    for tr in stream:
        if tr.stats.channel[-1] == 'Z':
            ...
            axZ.minorticks_on()
            axZ.grid(True, which='major', axis='x', linestyle='-', linewidth=0.7)
            ...
            # Enable both major and minor grids
            axN.minorticks_on()
            axN.grid(True, which='major', axis='x', linestyle='-', linewidth=0.7)
            ...
            # Enable both major and minor grids
            axE.minorticks_on()
            axE.grid(True, which='major', axis='x', linestyle='-', linewidth=0.7)
    ...
    with raspberry.seis_output:
        ...
        if stream:
            ...
            if P_arrival is not None and len(P_arrival) > 0:
                print(f"{P_arrival[0].name} arrival at {station_name}: {origin_time + P_arrival[0].time} s")
            ...
            if S_arrival is not None and len(S_arrival) > 0:
                print(f"{S_arrival[0].name} arrival at {station_name}: {origin_time + S_arrival[0].time} s")
        ...
        if sed_stream:
            ...
            if P_arrival is not None and len(P_arrival) > 0:
                print(f"{P_arrival[0].name} arrival at {sed_station_name}: {origin_time + P_arrival[0].time} s")
                plot_phase(axes=[axZ, axN, axE], phase_time=origin_time + P_arrival[0].time, 
            ...
            if S_arrival is not None and len(S_arrival) > 0:
                print(f"{S_arrival[0].name} arrival at {sed_station_name}: {origin_time + S_arrival[0].time} s")
                plot_phase(axes=[axZ, axN, axE], phase_time=origin_time + S_arrival[0].time, 
